...
<div class="card" style="margin-bottom: 24px; padding: 24px;">
    <form id="search-form" method="GET" action="" onsubmit="return false;">
        <!-- BARIS 1: INPUT FILTER (GRID RAPI) -->
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 18px; margin-bottom: 20px;">
            <div>
                <label for="tgl" style="font-weight: 600; font-size: 13px; color: #334155; margin-bottom: 6px; display: block;">
                    📅 Filter Tanggal:
                </label>
                <input type="date" id="tgl" name="tgl" value="<?php echo h($tgl === 'all' ? '' : $tgl); ?>" style="width: 100%; height: 42px; margin-bottom: 0;">
            </div>

            <div style="grid-column: span 2;">
                <label for="q" style="font-weight: 600; font-size: 13px; color: #334155; margin-bottom: 6px; display: block;">
                    🔍 Pencarian Real-Time:
                </label>
                <input type="text" id="q" name="q" value="<?php echo h($q); ?>" placeholder="Cari berdasarkan nama, PIN, departemen, atau waktu..." style="width: 100%; height: 42px; margin-bottom: 0;" autocomplete="off">
            </div>

            <div>
                <label for="status" style="font-weight: 600; font-size: 13px; color: #334155; margin-bottom: 6px; display: block;">
                    Status Absensi:
                </label>
                <select id="status" name="status" style="width: 100%; height: 42px; margin-bottom: 0;">
        ...
        <!-- BARIS 2: ACTION BUTTONS & SHORTCUTS (BARIS BAWAH TERATUR) -->
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; padding-top: 16px; border-top: 1px solid #f1f5f9;">
            <!-- Shortcut Tanggal (Kiri) -->
            <div style="display: flex; gap: 8px; align-items: center;">
                <span style="font-size: 12px; font-weight: 600; color: #64748b; margin-right: 4px;">Pintasan:</span>
                <button type="button" class="btn" style="background:#f1f5f9; color:#0f172a; font-size:13px; padding: 8px 14px; border:1px solid #e2e8f0;" onclick="setToday()">🗓️ Hari Ini</button>
            ...
            <!-- Tombol Kontrol Sync & Export (Kanan) -->
            <div style="display: flex; gap: 10px; align-items: center;">
                <button type="button" id="btn-toggle-sync" class="btn" style="background:#f8fafc; color:#334155; font-size:13px; border:1px solid #cbd5e1; padding: 9px 16px;" onclick="toggleAutoSync()">
                    <span id="sync-icon">🟢</span> <span id="sync-text">Auto-Sync (5s)</span>
                </button>
                
                <a id="btn-export" href="<?php echo h($export_url); ?>" class="btn btn-success" style="padding: 9px 18px; font-size: 13px;">
                    📊 Export ke Excel
                </a>
            </div>
        </div>
